  ...
  export default {
    ...
    methods: {
      ...
      async getExaminee(){
        ...
        this.examinee.admission = response.data.admission
        this.examinee.rank.A = response.data.ranking==='正取'?true:false
        this.examinee.rank.B = this.examinee.rank.A===true?false:true
        this.examinee.name = response.data.name
        this.examinee.examineeNumber = response.data.examineeNumber
      },
      async patchExaminee(){
        const response = await axios.patch('/examinee/'+this.examineeId,{
    ...
    <div class="bg-slate-500 flex justify-center items-center mb-">
        <div class=" flex flex-col bg-white text-black w-128 h-68 z-50 p-4 rounded-lg shadow-md transition-all">
          ...
          <div class="flex items-end justify-end">
            ...
            <button class="mt-10 text-lg  bg-[#41414E] text-white w-20 h-10 rounded-lg mb-4 py-2" @click="patchExaminee">
              儲存
            </button>
          </div>
        </div>
